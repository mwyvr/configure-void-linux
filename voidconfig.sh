...
initial_update_and_firmware() {
	if ! [ -f "$INSTALL_DIR/_sys_updated" ]; then
		...
		# 		fi
		#
		# 		VARMILO=$(lsmod | grep ath12k)
		# 		if ! test -z "$VARMILO"; then
...
setup_users() {
	echo "User Setup"
	if ask "Configure groups for existing users?" Y; then
		while true; do
...
EOF
	...
	if [[ $INSTALL_TYPE = "desktop" ]]; then
		...
		$INSTALLER flatpak
		# system, not user
		# su $TRUSTED_USER -c "flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo"
		# Fonts
...
EOF
		...
		# Wayland specific with some X support
		$INSTALLER xf86-input-evdev xinput libinput libinput
		# TODO trim this list down and add Roboto Mono
	fi
	...
	# bluetooth
	# https://docs.voidlinux.org/config/bluetooth.htmlhttps://codeberg.org/dwl/dwl.git
	if [[ $INSTALL_TYPE = "desktop" ]]; then
	...
	# set up to compile dwl window manager
	# dwl
...
dwl() {
	...
	mkdir -p src
	cd $TRUSTED_USER/src
	$USERCOMMAND -c "git clone https://codeberg.org/dwl/dwl.git"
	$INSTALLER foot wlroots wlroots-devel wayland wayland-devel wayland-protocols libinput libinput-devel xorg-server-xwayland

	# for somebar
	$INSTALLER meson cairo cairo-devel pango pango-devel
	$INSTALLER firefox
	cd $INSTALL_DIR
}

quality_of_life() {
	$INSTALLER htop rsync chezmoi neofetch
	# all for neovim
